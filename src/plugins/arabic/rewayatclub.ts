    ...
    loadedCheerio('.row--dense').each((idx, ele) => {
        loadedCheerio(ele).find('.v-sheet--outlined').each((idx, ele) => {
            ...
            const novelUrl = loadedCheerio(ele)
                ...
                ?.trim()
                .replace(/^\/*/, '') || '';
            const novelUrlMatch = novelUrl.replace('/\d+$|-/g', ' ')
            const imageRaw = loadedCheerio('body script:contains("__NUXT__")').text();
            const imageUrlRegex = /poster_url:"\\u002F([^"]+)"/g;
            const imageUrls: string[] = [];
            let match: RegExpExecArray | null;

            while ((match = imageUrlRegex.exec(imageRaw)) !== null) {
                imageUrls.push(match[1].replace(/\\u002F/g, '/').replace(/%20/g, ' ').replace(/^\/*/, ''));
            }
            
            let novelCover = defaultCover;
            
            for (const imageUrlShort of imageUrls) {
                if (imageUrlShort.includes(novelUrlMatch)) {
...
console.log(`Novel Cover: ${novelCover}`);
  ...
  async parsePage(novelPath: string, page: string): Promise<Plugin.SourcePage> {
    ...
    const chaptersinfo: { chapterName: string; chapterUrl: string; releaseTime: string; chapterNumber: number }[] = [];
    loadedCheerio('div[role="list"] a').each((i, el) => {
      ...
      const chapternumber = loadedCheerio(el).find('div.v-avatar--tile span.v-chip__content span').text();
    ...
    const pagecount = loadedCheerio('.v-select__selections div.v-select__selection--comma').text();
