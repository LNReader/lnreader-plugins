...
class Sunovels implements Plugin.PagePlugin {
  ...
  parseNovels(loadedCheerio: CheerioAPI): Plugin.NovelItem[] {
    ...
    loadedCheerio('body div.link').each((idx, ele) => {
      loadedCheerio(ele)
        .find('div.card')
        .each((idx, ele) => {
          const novelName = loadedCheerio(ele).find('div.header').text().trim();
          const novelUrl =
            loadedCheerio(ele).find('a').attr('href')?.trim().slice(1);
          let novelCover = 
          loadedCheerio(ele).find('img').attr('src')?.trim();
          const novel = {
            name: novelName,
            cover: novelCover,
            path: novelUrl,
  ...
  async searchNovels(
    ...
    const searchUrl = 'https://dilar.tube/api/mangas/search';
