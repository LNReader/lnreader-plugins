...
class ArchiveOfOurOwn implements Plugin.PluginBase {
  id = 'archiveofourowntest3';
  name = 'Archive Of Our Own test3';
  version = '1.0.1';
  ...
  async parseNovel(novelUrl: string): Promise<Plugin.SourceNovel> {
    ...
    const loadedCheerio = parseHTML(body);
    const baseUrl = new URL(novelUrl, this.site);
    baseUrl.pathname += '/navigate';
    const dateresult = await fetchApi(new URL(baseUrl).toString());
    const date = await dateresult.text();
    const loadeddate = parseHTML(date);
  ...
  loadedCheerio('ol.chapter').each((i, selectEl) => {
    loadedCheerio(selectEl)
      ...
      .each((i, el) => {
        ...
        const chapterUrl = chapterUrlCode ? `${novelUrl}/chapters/${chapterUrlCode}` : '';

        const publicationDateText = chapterEl.find('span.datetime').text().replace(/[()]/g, '').trim();
        const releaseTime = publicationDateText ? new Date(publicationDateText).toISOString() : '';
        

        if (chapterUrl) {
          // Use chapterName if it's empty, fall back to the novel title
          if (chapterName === '') {
          ...
          chapterItems.push({
            ...
            releaseTime: releaseTime // Include publication date
          });
        }
      });
  });
