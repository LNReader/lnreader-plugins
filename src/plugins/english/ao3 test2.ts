...
class ArchiveOfOurOwn implements Plugin.PluginBase {
  ...
  async parseChapter(chapterUrl: string): Promise<string> {
    ...
    loadedCheerio('h3.title a').each((i, el) => {
      ...
      element.parent().html(element.text() + '<br>' + element.parent().contents().eq(1).text().trim());
    });
  ...
  //   dateIncrements: filters.dateIncrements.value,
  //   getCombinedValue: function() {
  //     return `${this.date.value}+${this.dateIncrements.value}`;
  //   },
  // };
  filters = {
    sort: {
