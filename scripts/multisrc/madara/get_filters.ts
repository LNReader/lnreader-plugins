...
function getFilters(name: string, html: string) {
	...
	filters.filters["op"].label = statusDiv.find("option").eq(1)
		.text().replace("AND", "").replace("(", "").replace(")", "").trim();
	...
	orderByDiv.find("a").each((i, el) => {
		const option = {
			label: $(el).text().trim(),
			value: decodeURI(($(el).attr("href")?.split("=").length == 3 ? $(el).attr("href")?.split("=")[2] : "") || "")
		};
...
async function askGetFilter(){
	...
	await readline.question('Enter the id of the site (same one as in sources.json): ', async (name: string) => {
		await readline.question(EREASE_PREV_LINE + 'Do you want to get the filters from a URL or the html text? (if url dosen\'t work try html) (url/html): ', async (method: string) => {
			if (method.toLowerCase() === "url") {
				await readline.question(EREASE_PREV_LINE + 'Enter the URL (same one as in sources.json): ', async (url: string) => {
					try {
				...
				process.stdout.write(EREASE_PREV_LINE + `Enter the html text from the page at {sourceSite}/?s=&post_type=wp-manga (at the end press ENTER then press CTRL+C): `)
				let html = "";
				readline.on('SIGINT', () => {
