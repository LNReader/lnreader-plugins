...
class ChireadsPlugin implements Plugin.PluginBase {
    ...
    async getCheerio(url: string): Promise<CheerioAPI> {
        const r = await fetchApi(url, {headers: { 'Accept-Encoding': 'deflate' }});
    ...
    async popularNovels(pageNo: number, { filters, showLatestNovels }: Plugin.PopularNovelsOptions): Promise<Plugin.NovelItem[]> {
        ...
        if (showLatestNovels || tag !== "all") {
            ...
            for (let i = 0; i < loop; i++) {
                if (i === 1)
                    $ = await this.getCheerio(this.site + "/category/original/page/" + pageNo);
                let romans = $(".romans-content li");
                if (!romans.length)
                    romans = $("#content li");
                romans.each((i, elem) => {
                    const novelName = $(elem).contents().find("div").first().text().trim();
                    const novelCover = $(elem).find("div").first().find("img").attr("src");
                    const novelUrl = $(elem).find("div").first().find("a").attr("href")
    
                    if (novelUrl){
                        novel = { name: novelName, cover: novelCover, path: novelUrl.replace(this.site, "") };
                        novels.push(novel);
                    }    
                });
            }
            ...
            const populaire = $(":contains(\"Populaire\")").last().parent().next().find("li > div")
            if (populaire.length === 12) // pc
            {
                ...
                populaire.each((i, elem) => {
                    if (i % 2 === 0)
                        novelCover = $(elem).find("img").attr("src");
                    else {
                        ...
                        novel = { name: novelName, cover: novelCover || defaultCover, path: novelUrl.replace(this.site, "") };
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
        ...
        novel.name = $(".inform-product-txt").first().text().trim() || $(".inform-title").text().trim();
        novel.cover = $(".inform-product img").attr("src") || $(".inform-product-img img").attr("src") || defaultCover;
        novel.summary = $(".inform-inform-txt").text().trim() || $(".inform-intr-txt").text().trim();

        let infos = $("div.inform-product-txt > div.inform-intr-col").text().trim() || $("div.inform-inform-data > h6").text().trim();
    ...
    async searchNovels(searchTerm: string, pageNo: number): Promise<Plugin.NovelItem[]> {
        ...
        while ( !finised )
        {
            await this.popularNovels(i, { showLatestNovels: true, filters: undefined } ).then((res) => {
                if (res.length === 0)
                    finised = true;
                novels.push(...res);
            });
