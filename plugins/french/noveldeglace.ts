...
class NovelDeGlacePlugin implements Plugin.PluginBase{
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel> {
        ...
        novel.name = ($("div.entry-content > div > strong")[0]
            .nextSibling as Text | null)
            ?.nodeValue
            ?.trim() || "Untitled";  

        novel.cover = $(".su-row > div > div > img")
            .attr("src") || defaultCover;
        
        let novelInfos = $("div[data-title=Tomes] >")
            .toArray();
        novelInfos.pop();
        novelInfos.shift();
        novel.summary = $("div[data-title=Synopsis]")
        .text().trim() + 
        ...
        let status = $("strong:contains('Statut :')")
            .parent()
            .attr("class");
        switch (status) {
            case "type etat0":
        ...
        const volumes = $("div[data-title=Tomes] > div")
            .last()
            .contents();
            ...
            $(el)
                ...
                .each((chapterIndex: number, el) => {
                    ...
                    else // has parts that needs to be added individually
                    {
                        ...
                        items?.shift();
                        const dates: string[] = [];
                        items?.forEach((item) => { // there is a date on every publish parts
