...
class LightNovelVN implements Plugin.PagePlugin {
    ...
    async popularNovels(pageNo: number, options: Plugin.PopularNovelsOptions<Filters>): Promise<Plugin.NovelItem[]> {
        ...
        loadedCheerio(".flex.flex-col[itemtype='https://schema.org/Book']").each((idx, ele) => {
            ...
            const novelUrl = loadedCheerio(ele).find('h3[itemprop="name"] > a').attr('href');
            if(novelUrl)
            novels.push({
                name: novelName,
                cover: novelCover,
    ...
    async searchNovels(searchTerm: string, pageNo: number): Promise<Plugin.NovelItem[]> {
        ...
        const result: SearchedResult = await fetchApi(
            url,
            {
                method: "POST",
                body: formData,
        ...
        const novels: NovelItem[] = result.data?.map(item => {
            return {
                name: item.name,
                path: '/truyen/' + item.slug,
                cover: (this.site + item.coverUrl).replace('default.jpg', '150.jpg?w=256&q=')
            }
        }) || [];
        return novels;
    }
