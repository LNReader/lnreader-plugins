...
class TruyenConect implements Plugin.PagePlugin {
    ...
    async getVolumes (firstChapterUrl: string) {
        ...
        if(data.eps_selector){
            volumeList.volumes = this.parseVolumes(parseHTML(data.eps_selector as string))
        }
        return volumeList
    }
        ...
        loadedCheerio(".post-content_item").each(function () {
            ...
            switch (detailName) {
                ...
                case "Trạng thái":
                    ...
                    else if(status === 'Đang tiến hành') novel.status = NovelStatus.Ongoing
    ...
    async parsePage(novelPath: string, page: string): Promise<Plugin.SourcePage> {
        ...
        if(volumeIndex >= volumeList.volumes.length) throw new Error("Không tìm thấy volume");
        ...
        await this.sleep(1000);
        const res: {err: number, html: string} = await fetch(chaptersUrl).then(r => r.json());
