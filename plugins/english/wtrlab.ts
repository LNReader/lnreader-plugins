...
class WTRLAB implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    if(showLatestNovels){
      ...
      const novels: Plugin.NovelItem[] = loadedCheerio(".serie-item")
        .map((index, element) => ({
          name: loadedCheerio(element).find(".title-wrap > a").text().replace(loadedCheerio(element).find(".rawtitle").text(), "") || "",
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    const chapterContent = JSON.stringify(jsonData.props.pageProps.serie.chapter_data.data.body);
