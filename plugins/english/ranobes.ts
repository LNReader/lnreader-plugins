...
class RanobesPlugin implements Plugin.PagePlugin {
    ...
    async parseNovel(novelPath: string): Promise<Plugin.SourceNovel & { totalPages: number; }> {
        ...
        novel.status = loadedCheerio('li:contains("Status") span')
            .text() === 'Ongoing'
                ? 'Ongoing'
                : 'Completed';
        ...
        const chaptersHtmlToString = await fetchApi(chapterListUrl).then(r => r.text());
    ...
    async parsePage(novelPath: string, page: string): Promise<Plugin.SourcePage> {
        ...
        const firstUrl = this.site + 
            '/chapters' +
            pagePath.replace('novels/', '');
