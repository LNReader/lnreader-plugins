...
class NovelsOnline implements Plugin.PluginBase {
    ...
    async popularNovels(
        ...
        if (filters.genre.value !== '')
            link += `/category/${filters.genre.value}/`;
        else
            link += `/top-novel/`;
        ...
        loadedCheerio('.top-novel-block').each((i,el) => {
            ...
            const novelCover = loadedCheerio(el).find('.top-novel-cover img').attr('src');
        ...
        $('.novel-detail-item').each((i,el) => {
            ...
            switch(detailName){
                case "Description":
                    ...
                    break;
                case "Genre":
                    novel.genres = detail.find('li')
                        .map((_, el) => $(el).text())
                        .get()
                        .join(', ');
                    break;
                case "Author(s)":
                    novel.author = detail.find('li')
                        .map((_, el) => $(el).text())
                        .get()
                        .join(', ');
                    break;
